## 嵌入式硬件方向实习 · ARM SoC 架构笔记

### 一、什么是 ARM SoC

SoC（System on Chip）= 把整个系统放到一颗芯片里。不同于 STM32（MCU，微控制器），ARM SoC 通常指 **应用处理器级别的芯片**（如手机里的骁龙、树莓派的 BCM2837、全志 H3 等）。
...
| 内存 | 片上 SRAM（KB~MB 级） | 外挂 DDR（GB 级） |
